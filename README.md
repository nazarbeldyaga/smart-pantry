...
Застосунок дозволяє створити віртуальну копію вашої реальної комори, холодильника та морозильної камери. Але замість того, щоб бути просто "списком продуктів", Smart Pantry виступає як проактивний асистент: він активно допомагає вам використовувати наявні запаси, завчасно нагадуючи про закінчення термінів придатності.
...
1. npm install
2. Створити файл .env в package/api проекту з наступними змінними оточення:

   ```
   PORT = порт сервера (наприклад, 3000)
   JWT_SECRET = секретний ключ для JWT токенів

   DB_HOST = хост бази даних (localhost)
   DB_PORT = порт бази даних (наприклад, 5432)
   DB_USERNAME = ім'я користувача бази даних (myuser)
   DB_PASSWORD = пароль користувача бази даних (mypassword)
   DB_NAME = назва бази даних (smart_pantry)
   ```

3. Створити файл .env в package/web проекту з наступними змінними оточення:
   ```
   PORT = порт фронтенд сервера (наприклад, 3001)
   VITE_API_URL = URL вашого бекенд сервера (http://localhost:3000)
   ```
4. Запустити PostgreSQL сервер, виконавши скрипт docker compose up -d (є готовий у package.json), переокнавшись що docker desktop запущений.
5. Запустити бекенд, фронтенд готовими скриптами в package.json.
